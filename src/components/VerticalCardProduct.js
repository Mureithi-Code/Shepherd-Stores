...
const VerticalCardProduct = ({ category, heading }) => {
  ...
  const fetchData = async () => {
    if (!category || typeof category !== 'string') {
      console.warn("❗ Invalid or missing 'category' passed to VerticalCardProduct.")
      return
    }
          ...
          loadingList.map((_, index) => (
            <div key={index} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-sm shadow'>
              ...
              <div className='p-4 grid gap-3'>
                <h2 className='h-4 bg-slate-200 animate-pulse rounded-full'></h2>
                <p className='h-4 bg-slate-200 animate-pulse rounded-full'></p>
                <div className='flex gap-3'>
                  <p className='h-4 w-full bg-slate-200 animate-pulse rounded-full'></p>
                  <p className='h-4 w-full bg-slate-200 animate-pulse rounded-full'></p>
                </div>
                <button className='h-8 bg-slate-200 animate-pulse rounded-full'></button>
              </div>
            </div>
          ))
        ) : data.length === 0 ? (
